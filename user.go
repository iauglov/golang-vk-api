...
func (client *VKClient) UsersGetStringIDs(users []string) ([]*User, error) {
	idsString := make([]string, 0, len(users))
	for _, user := range users {
		idsString = append(idsString, user)
	}
	v := url.Values{}
	v.Add("user_ids", strings.Join(idsString, ","))
